...
const Philosophy = () => {
  return (
    <section className="py-12 space-y-10 relative z-10 overflow-hidden">
      ...
      {/* Subtext */}
      <p data-aos="fade-up" className="text-center w-full max-w-3xl text-2xl mx-auto text-textColor z-10">
      ...
      <div className="container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-20 z-10 gap-y-16">
        {dataList.map((item) => (
          ...
          data-aos="zoom-in"
            key={item.id}
            className="bg-white rounded-3xl shadow-xl py-6 px-16 text-center transition duration-300 hover:shadow-2xl hover:-translate-y-1 z-10"
          >
            <div className="flex justify-center mb-4 -mt-20 ">
              <img
                src={item.img}
                alt={item.name}
                className="h-32 w-32 p-2 rounded-full bg-[#f3f3f3] pointer-events-none"
              />
            </div>
